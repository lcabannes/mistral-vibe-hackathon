...
# --- Generic secret storage (privacy-routing vault) -------------------------
# Same service as API keys, but accounts are namespaced by the caller (e.g.
# "vibe-secret:[REDACTED:aws-access-key-id:3]") and there is no cache: vault
# lookups are rare and the values must never linger in extra copies.

_SECRET_ACCOUNT_PREFIX = "vibe-secret:"


def set_secret_in_keyring(name: str, value: str) -> bool:
    """Store a vault secret; returns False if the keychain is unavailable."""
    if _is_keyring_disabled():
        return False
    try:
        _set_password(_KEYRING_SERVICE, f"{_SECRET_ACCOUNT_PREFIX}{name}", value)
    except KeyringError:
        return False
    return True


def get_secret_from_keyring(name: str) -> str | None:
    if _is_keyring_disabled():
        return None
    try:
        return _get_password(_KEYRING_SERVICE, f"{_SECRET_ACCOUNT_PREFIX}{name}")
    except _PasswordNotFoundError:
        return None
    except KeyringError:
        return None
...
def delete_secret_from_keyring(name: str) -> None:
    if _is_keyring_disabled():
        return
    try:
        _delete_password(_KEYRING_SERVICE, f"{_SECRET_ACCOUNT_PREFIX}{name}")
